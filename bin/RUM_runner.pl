...
if($postprocess eq "false") {
    ...
    for($i=1; $i<=$numchunks; $i++) {
        ...
        if($i == $numchunks) {
            ...
            $shellscript = $shellscript . "perl $scripts_dir/merge_nu_stats.pl $output_dir -n $numchunks -chunk_ids_file $output_dir/restart.ids >> $output_dir/mapping_stats.txt 2>> $output_dir/PostProcessing-errorlog || exit 1\n";
