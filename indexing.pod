...
=head3 Downloading the genome sequence from the genome downloads page
...
=head3 Downloading the genome sequence using the table browser
...
If there are many annotation tracks, such as for mouse where there are
more than ten, you probably shouldn't include them all, by default we
have been using just three of them for mouse: refseq, ucsc and
aceview. Using all tracks would create a transcript db that is too
complex, it's better to let blat map the reads for transcripts that
are not present in the db rather than to expand the db too much with
false transcripts.
...
=item http://www.ensembl.org/

=item http://metazoa.ensembl.org/

=item http://fungi.ensembl.org/

=item http://plants.ensembl.org/

=item http://bacteria.ensembl.org/
...
I<Note>: edit rum.config_NAME if you have moved any scripts, executables,
 or indexes from their default locations.
