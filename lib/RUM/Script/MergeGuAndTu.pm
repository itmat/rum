...
sub run {
    ...
    # Open an iterator over records that look like:
    #
    #   { 
    #     gu  => RUM::Mapper with unique genome alignments
    #     tu  => RUM::Mapper with unique transcriptome alignments,
    #     gnu => RUM::Mapper with non-unique genome alignments
    #     gnu => RUM::Mapper with non-unique transcriptome alignments
    #   }
    # 
    # At least one of the keys will be present, but some of them might
    # be missing.
    #
    # Assuming all of the input files are sorted by the order of the
    # read, the records will pop off the iterator in sorted order,
    # with one record per read.
    my $gu_iter =  unique_iter($self->{gu_in_fh},  'gu');
    my $tu_iter =  unique_iter($self->{tu_in_fh},  'tu');
    my $gnu_iter = unique_iter($self->{gnu_in_fh}, 'gnu');
    my $tnu_iter = unique_iter($self->{tnu_in_fh}, 'tnu');

    my $input = $gnu_iter->merge(
        others => [$tnu_iter, $gu_iter, $tu_iter],
        cmp_fn => \&RUM::Mapper::cmp_read_ids,
        group_fn => \&group_mappers_by_source);

    # Open output sinks for the the unique and non-uniqe mappers
    my $unique_io = RUM::RUMIO->new(-fh => $self->{bowtie_unique_out_fh}, strand_last => 1);
    my $cnu_io    = RUM::RUMIO->new(-fh => $self->{cnu_out_fh}, strand_last => 1);
        
  READ: while (my $mappers = $input->next_val) {

        my $gu  = $mappers->{gu};
        my $tu  = $mappers->{tu};

        # If we have any non-unique mappers against either the
        # genome or transcriptome, skip it.
        next READ if $mappers->{gnu} || $mappers->{tnu};
